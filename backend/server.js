  ...
  app.get("/getAllNotes", express.json(), async (req, res) => {
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        // Find all notes for current user
        const collection = db.collection(COLLECTIONS.notes);
    ...
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        if (!data) {
          return res
            .status(404)
            .json({ error: "Unable to find or delete note with given ID." });
        }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
    try {
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "Atleast one of title or content is required." });
      }
      ...
      if(!title) {
        ...
        const collection = db.collection(COLLECTIONS.notes);
            ...
            //Check if noteId belongs to user 
            if (!data) {
            return res
              .status(404)
              .json({ error: "Unable to find or edit note with given ID." });
            }
                //Update the note
